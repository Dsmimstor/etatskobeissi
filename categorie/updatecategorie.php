...
if (!isset($_SESSION["users"])) {
    ...
    if (isset($_POST["btn_update"])) {
        ...
        $resultfour["codecat"] = $_POST["abreviation"];
        $resultfour["designationcat"] = $_POST["names"];
        $idfour = $_POST["id"];

        $RSqls = "UPDATE `tcategories` SET
        `codecat`=:abreviation,
        `designationcat`=:names  WHERE `id`=$idfour";
        ...
        $id = $_GET["id"];
        $RSqls = "SELECT * FROM `tcategories` WHERE `id`  = '$id' ";
        $query = $conn->prepare($RSqls);
        $query->execute();
...
<head>
    ...
    if ($excecuteIsOk == "succes") {
        echo
            "<script> alert('La Categorie a été modifié'); location.href='listecategorie.php'; </script>";
    }
    if ($excecuteIsOk == "error") {
        "<script> alert('Echec de la modification de la categorie'); </script>";
        }?>
                        ...
                        <div class="modal-body">
                            <div class="container-fluid">
                                ...
                                <div class="form-group input-group">
                                    ...
                                    <input type="text" onkeydown="upperCaseFun(this)" style="width:350px; text-align: left;" class="form-control" id="abreviation" name="abreviation"  value="<?php echo $resultfour["codecat"]; ?>">
                                </div>
                                <div class=" form-group input-group">
                                    <span class="input-group-addon" style="width:150px;">Désignation :</span>
                                    <input type="text"  onkeydown="upperCaseFun(this)" style="width:350px;" class="form-control" id="names" name="names" value="<?php echo $resultfour["designationcat"]; ?>">
                                </div>
                          <div class="modal-footer">
                            <button type="submit" class="btn btn-success " name="btn_update"><span class="glyphicon glyphicon-edit"></span> Modififer</button>
                            <a href="listecategorie.php" class="btn btn-default"><span class="glyphicon glyphicon-remove"></span> Annuller</a>
                        </div>
                    </div>
                </div>
