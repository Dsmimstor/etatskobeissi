...
$datedebut = isset($_GET["from_date"]) ? $_GET["from_date"] : '';
$datefin = isset($_GET["to_date"]) ? $_GET["to_date"] : '';
$etatstock="stocker";
...
// Vérifier si $datedebut est vide ou invalide
if (empty($datedebut) || !validateDate(substr($datedebut, 0, 10))) {
    $date = new DateTime();
    $datemy = $date->format('Y-m');
    $from_date = $datemy . "-01"; // Début du mois courant
    $to_date = $date->format('Y-m-d'); // Date actuelle
} else {
    // Si $datedebut n'est pas vide et valide, utiliser les dates fournies
    $from_date = substr($datedebut, 0, 10); // Prendre les 10 premiers caractères
    $to_date = substr($datefin, 0, 10); // Prendre les 10 premiers caractères

    // Vérifier si $to_date est valide
    if (!validateDate($to_date)) {
        $to_date = (new DateTime())->format('Y-m-d'); // Date actuelle si invalide
    }
}
...
if (!isset($_SESSION["users"])) {
    ...
    $RSqlfacture = "SELECT d.*, c.* FROM `tdocuments` d INNER JOIN `tdetail_document` c ON d.id = c.id_document WHERE d.`typesfacture` = '$idc' and c.`etatstock` = '' ORDER BY nfacture ASC";
    $queryfacture = $conn->prepare($RSqlfacture);
    $queryfacture->execute();
    $resultfacture = $queryfacture->fetchAll(PDO::FETCH_ASSOC);
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST["validateBtn"])) {
        ...
        if (!empty($designation) && !empty($quantity) && !empty($movementType) && !empty($movementDate) && !empty($operateur)) {
            $sqle = "INSERT INTO tstocks (dates, designations, quantites, mouvements, operateur,nfacture)
                    VALUES (:movementDate, :designation, :quantity, :movementType, :operateur, :nfacture)";
            $stmte = $conn->prepare($sqle);
            $stmte->bindValue(':nfacture', $nfacture, PDO::PARAM_STR);
            ...
            $sqlel = "INSERT INTO tlistestocks (id_fac, quantite)
            VALUES (:nfacture, :quantity)";
            $stmtel = $conn->prepare($sqlel);
            $stmtel->bindValue(':nfacture', $nfacture, PDO::PARAM_STR);
            $stmtel->bindValue(':quantity', $quantity, PDO::PARAM_INT);


            $ids = $_POST['ids'];
            $RSqlss = "UPDATE `tdetail_document` SET
            etatstock =:etatstock WHERE `id`=$ids";
            $stmtss = $conn->prepare($RSqlss);
            $stmtss->bindValue(':etatstock', $etatstock, PDO::PARAM_STR);
            ...
            if ($stmte->execute()) {
                ...
                $stmtel->execute();
                $stmtss->execute();
                echo "La Rentrée a été enregistrée";
                header("Location: entrestockud.php");
            } else {
...
<head>
    ...
    <script>
        function tris(id) {
            ...
            const url = `entrestockdd.php?id=${encodeURIComponent(id)}
    ...
    <div class="container">
        ...
        <div class="card">
            ...
            <div class="search-form " style="margin:10px;">
                <div class="search-form">
                    <div class="bordered-div ">
                        <form action="#" method="GET" style="display: flex; margin:10px;">
                            ...
                            <input type="date" name="from_date" value='<?= htmlspecialchars($from_date, ENT_QUOTES, 'UTF-8') ?>' class="form-control" placeholder="From Date" style="width:150px;">
                            &nbsp;&nbsp;&nbsp;
                            <span class="input-group-addon" style="width:70px;">AU :</span>
                            <input type="date" name="to_date" value='<?= htmlspecialchars($to_date, ENT_QUOTES, 'UTF-8') ?>' class="form-control" placeholder="To Date" style="width:150px;">
                            &nbsp;&nbsp;&nbsp;
                        ...
                        <tr>
                            <form method="POST" action="">
                                <th scope="col" class="T2" style="width: 90px;">
                                    <a href="/stockentree/entrestockdd.php" name="datebtn" class="fa fa-play fa-2x rotate--270" aria-hidden="true"></a> &nbsp;Date
                                </th>
                                <th scope="col" class="T7" style="width: 110px;">
                                    <a href="/stockentree/entrestockudf.php" name="datebtn" class="fa fa-play fa-2x rotate--270" aria-hidden="true"></a> &nbsp;N°Facture
                                </th>
                                <th scope="col" class="T7" style="width: 300px;">
                                    <a href="/stockentree/entrestockdda.php" name="datebtn" class="fa fa-play fa-2x rotate--270" aria-hidden="true"></a>&nbsp;Designation
                                </th>
                                ...
                                <th scope="col" class="T4" style="width: 200px;">
                                    <a href="/stockentree/entrestockudo.php" name="datebtn" class="fa fa-play fa-2x rotate--270" aria-hidden="true"></a> &nbsp;Operateur
                                </th>
                    ...
                    <div class="modal-content" style="width: 700px;">
                        ...
                        <form id="modalForm" method="POST">
                            <div class="form-group input-group" hidden>
                                ...
                                <input type="text" readonly style="width:450px;" class="form-control" id="ids" name="ids">
                            </div>
                            <div class="form-group input-group">
                                <span class="input-group-addon" style="width:170px;">N° Facture:</span>
                                <input type="text" style="width:350px;" readonly onkeydown="upperCaseFun(this)" class="form-control" id="nfacture" name="nfacture">
                                <button type="button" id="facturebtn" class="btn btn-primary">Liste Facture</button>
                            </div>
                            <div class="form-group input-group">
                                <span class="input-group-addon" style="width:170px;">Désignation:</span>
                                <input type="text" style="width:450px;" readonly onkeydown="upperCaseFun(this)" class="form-control" id="designation" name="designation">
                            </div>
                            <div class="form-group input-group">
                            ...
                            <div class="form-group input-group">
                                ...
                                <input type="text" style="width:450px;" readonly class="form-control" id="movementType" name="movementType">
                            </div>
                            <div class="form-group input-group">
                ...
                <div id="modal2" class="modal">
                    <div class="modal-content" style="width: 800px;">
                        ...
                        <table id="mouvementTable1">
                            <thead>
                                <tr>
                                    <th scope="col" class="T7" style="width: 110px;">ID</th>
                                    <th scope="col" class="T7" style="width: 110px;">N°Facture</th>
                                    <th scope="col" class="T7" style="width: 300px;">Designation</th>
                                    <th scope="col" class="T4" style="width: 90px;">Quantité</th>
                                    <th scope="col" class="T4" style="width: 70px;">Action</th>
                                    <th scope="col" class="T4" style="width: 30px;" hidden>ID</th>
                                </tr>
                            ...
                            <tbody id="table-body">
                                <?php foreach ($resultfacture as $resultfactures) : ?>
                                    <tr>
                                        <td style="text-align: center; font-family: sans-serif; padding: 3px;"><?= htmlspecialchars($resultfactures["id"]) ?></td>
                                        <td style="text-align: center; font-family: sans-serif; padding: 3px;"><?= htmlspecialchars($resultfactures["nfacture"]) ?></td>
                                        <td style="text-align: center; font-family: sans-serif; padding: 3px;"><?= htmlspecialchars($resultfactures["articles"]) ?></td>
                                        <td style="text-align: center; font-family: sans-serif; padding: 3px;"><?= htmlspecialchars($resultfactures["quantite"]) ?></td>
                                        <td style="font-family: sans-serif; text-align: center; padding: 3px;">
                                            <i class="fa fa-pencil fa-2x factureBtns" style="font-size:20px;"></i>
                                        </td>
                                        <td style="text-align: center; font-family: sans-serif; padding: 3px;" hidden><?= htmlspecialchars($resultfactures["id"]) ?></td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
                        ...
                        <div class="modal-footer">
                            <?php
                            $tooltipText = "Annuler";
                            echo '<a type="button" id="cancelBtn2" class="btn btn-primary" data-toggle="tooltip" data-placement="top" title="' . htmlspecialchars($tooltipText, ENT_QUOTES, 'UTF-8') . '"><span class="glyphicon glyphicon-plus" style="color:white"></span> ANNULER </a>';
                            ?>
                        </div>
                    </div>
                </div>
            ...
            <script>
                ...
                var facturebtn = document.getElementById("facturebtn");


                // Récupère les éléments <span> qui ferment les modaux
                var spans = document.getElementsByClassName("close");

                // Récupérez les boutons d'annulation
                ...
                // Lorsque l'utilisateur clique sur le bouton Annuler, ferme le modal
                ...
                $(document).ready(function() {
                    $('.factureBtns').on('click', function() {
                        ...
                        var nfacture = currentRow.find('td:eq(1)').text();
                        var designation = currentRow.find('td:eq(2)').text();
                        var quantity = currentRow.find('td:eq(3)').text();
                        var movementType = "Entré";
                        //  var operateur = currentRow.find('td:eq(4)').text();
                        // var id1 = currentRow.find('td:eq(7)').text();

                        // Fill the modal with the data
                        $('#ids').val(ids);
                        $('#nfacture').val(nfacture);
