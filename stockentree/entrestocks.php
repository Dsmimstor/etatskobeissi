...
if (!isset($_SESSION["users"])) {
    ...
    $TSql = "SELECT COUNT(*) AS nb_enregistrement FROM `tstocks` WHERE mouvements = :typefactures";
    if ($from_date && $to_date) {
        $TSql .= " AND dates BETWEEN :from_date AND :to_date";
    }
    $stmt = $conn->prepare($TSql);
    $stmt->bindValue(':typefactures', $typefactures, PDO::PARAM_STR);
    if ($from_date && $to_date) {
    ...
    $RSql = "SELECT * FROM `tstocks` WHERE mouvements = :typefactures";
    if ($from_date && $to_date) {
            ...
            <div class="search-form " style="margin:10px;">
                ...
                <table>
                    <thead>
                        ...
                        <th scope="col" class="T4" style="width: 90px;">Mouvement</th>
                        <th scope="col" class="T4" style="width: 200px;">Operateur</th>
