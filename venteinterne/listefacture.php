...
if (!isset($_SESSION["users"])) {
    ...
    //if (isset($_GET['typefactures'])) {
        $typefactures = "FA";; //$_GET["typefactures"];
    //}
    if ($typefactures == "T") {
        ...
        $RSql = "SELECT  c.designations as client,f.id, f.datefacture, f.typesbl, f.id_client, f.numeros, f.etatcmd, f.totalht, f.tfrais, f.ttransport FROM `tdocuments` f INNER JOIN `tclients` c ON f.id_client = c.id  WHERE datebl BETWEEN '$from_date' AND '$to_date' GROUP BY  c.designations, f.id, f.datefacture, f.etatcmd, f.id_client, f.nbonliv, f.numeros, f.totalht, f.tfrais, f.ttransport, f.typesbl ORDER BY datebl  LIMIT :premier, :parpage ";
        ...
        $TSql = "SELECT COUNT(*) AS nb_enregistrement FROM `tdocuments` d INNER JOIN `tclients` c ON d.id_client = c.id WHERE d.datefacture BETWEEN '$from_date' AND '$to_date' AND d.typesfacture='$typefactures'";
        ...
        $RSql = "SELECT  c.designations as client,f.id, f.datefacture, f.nfacture, f.etatcmd, f.id_client, f.observation, f.totalht, f.tfrais, f.ttransport FROM `tdocuments` f INNER JOIN `tclients` c ON f.id_client = c.id  WHERE datefacture BETWEEN '$from_date' AND '$to_date' AND typesfacture='$typefactures' GROUP BY  c.designations, f.id, f.datefacture, f.typesbl, f.id_client, f.observation, f.nfacture, f.typesfacture, f.totalht, f.tfrais, f.ttransport ORDER BY datefacture  LIMIT :premier, :parpage ";
        $stmt = $conn->prepare($RSql);
        $stmt->bindValue(':premier', $premier, PDO::PARAM_INT);
        $stmt->bindValue(':parpage', $parPage, PDO::PARAM_INT);
        $stmt->execute();
        ...
        <div class="well">
            <center>
                <div style="border: 1px solid black; margin: 5px;">
                    <h1 style="text-shadow: -2px -2px white,2px 2px #0F0604; color:#504846;">Liste des Factures</h1>
                </div>
                        ...
                        <tr>
                            ...
                            <td style="text-align: center; font-family: sans-serif; padding: 3px; ">
                                <span class="border border-primary" id="dates">
                                    <?= date('d-m-Y', strtotime($server["datefacture"])); ?>
                                </span>
                            </td>
                            ...
                            <td style="font-family: sans-serif ; text-align: center; padding: 3px; ">
                                <span class="border border-primary" id="ttcc">
                                    <?= $server["etatcmd"] ?>
                                </span>
                            </td>
                            <td style=" font-family: sans-serif ; text-align: center; padding: 3px; ">
                                <a type="button" onclick="test(<?php echo $server['id']; ?>, '<?php echo $server['etatcmd']; ?>')" class="edit"><i class="fa fa-search fa-2x" style="font-size:20px;"></i></a>
                              </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
