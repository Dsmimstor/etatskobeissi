...
if (!isset($_SESSION["users"])) {
    ...
    $RSqldev = "SELECT * FROM `tdevise` WHERE `id`  = '1' ";
    $querydev = $conn->prepare($RSqldev);
    $querydev->execute();
    $resultdev = $querydev->fetch(PDO::FETCH_ASSOC);
...
if (isset($_POST["btn_valider"])) {
    ...
    $query->bindValue(":dates", $_POST["datefacture"], PDO::PARAM_STR);
...
<head>
    ...
    <script>
        function valider(id) {
            ...
            const tauxechange = parseFloat(document.getElementById('tauxechange').value);
            ...
            if (confirm("Voulez-vous vusialiser la Facture ?")) {

                // Construire l'URL avec les paramètres de requête encodés
                const url = `reportpdf.php?id=${encodeURIComponent(id)}
                &tht=${encodeURIComponent(tht)}
                &frais=${encodeURIComponent(frais)}
                &transport=${encodeURIComponent(transport)}
                &datefacture=${encodeURIComponent(datefacture)}
                &tauxechange=${encodeURIComponent(tauxechange)}
                &client=${encodeURIComponent(client)}
                &email=${encodeURIComponent(email)}
                &telephone=${encodeURIComponent(telephone)}
                &adresse=${encodeURIComponent(adresse)}
                &nfacture=${encodeURIComponent(nfacture)}`;
    ...
    <div class="container">
        ...
        <div class="well">
            <div class="dv1" style="text-align:center; margin:10px; box-shadow: 2px -1px 3px ;">
                <h2 style="text-shadow: 1px 1px 2px brown, 0 0 1em brown, 0 0 0.2em brown;"> DETAIL FACTURE </h2>
            </div>
                    ...
                    <div class="p-4 flex-fill dv" style=" padding: 1px; ">
                        <div class="p-4 flex-fill dv" style=" padding: 1px; ">
                            <div class=" form-group1 input-group" style=" padding: 1px;">
                                ...
                                <input type="text" readonly onkeydown="upperCaseFun(this)" style="width:160px; text-align: left;" id="commande" name="commande" required value="<?php echo $resultcmd["ncommande"]; ?>">
                            </div>
                            <div class=" form-group1 input-group" style=" padding: 1px;">
                                <span class="input-group-addon" style="width:122px; text-align: left;">Date Cmde :</span>
                                <input type="date" readonly style="width:160px; " id="datecmd" name="datecmd" required value="<?php echo $resultcmd["datecmd"]; ?>">
                            </div>
                            <div class=" form-group1 input-group" style=" padding: 1px;">
                                <span class="input-group-addon" style="width:122px; text-align: left; ">N° Bon Liv :</span>
                                <input type="text" readonly onkeydown="upperCaseFun(this)" style="width:160px; text-align: left;" id="nbonliv" name="nbonliv" required value="<?php echo $resultcmd["nbonliv"]; ?>">
                            </div>
                            <div class=" form-group1 input-group" style=" padding: 1px;">
                                <span class="input-group-addon" style="width:122px; text-align: left;">Date Bon Liv :</span>
                                <input type="date" readonly style="width:160px; " id="datebl" name="datebl" required value="<?php echo $resultcmd["datebl"]; ?>">
                            </div>
                            <div class=" form-group1 input-group" style=" padding: 1px;">
                                <span class="input-group-addon" style="width:122px; text-align: left;">Date Fac :</span>
                                <input type="date" readonly style="width:160px; " id="datefacture" name="datefacture" required value="<?php echo $resultcmd["datefacture"]; ?>">
                            </div>
                            <div class=" form-group1 input-group" style=" padding: 1px;">
                                <span class="input-group-addon" style="width:122px; text-align: left; ">N° Facture :</span>
                                <input type="text" readonly onkeydown="upperCaseFun(this)" style="width:160px; text-align: left;" id="nfacture" name="nfacture" required value="<?php echo $resultcmd["nfacture"]; ?>">
                            </div>
                        </div>
                        ...
                        <input type="number" readonly hidden style="width:160px; text-align: left;" id="tauxechange" name="tauxechange" value="<?php echo $resultdev["tauxechange"]; ?>">
                    </div>
        ...
        <div class="modal-footer">
            <a type="button" onclick="valider(<?php echo $idc; ?>)" class="btn btn-warning">Inmprimer Facture</a>
            <a href="listecommande.php" class="btn btn-default"><span class="glyphicon glyphicon-remove"></span> Quitter</a>
        </div>
    </div>
    </div>
    <script>
        $(function(myFunction) {
            ...
            var id_tauxechange = parseFloat($("#tauxechange").val());
