...
if (!isset($_SESSION["users"])) {
    ...
    $typefactures = 'Encours'; //$_GET["typefactures"];
    //}
    if ($typefactures == "T") {
        ...
        $TSql = "SELECT COUNT(*) AS nb_enregistrement FROM `tdocuments` d INNER JOIN `tclients` c ON d.id_client = c.id WHERE d.datecmd BETWEEN '$from_date' AND '$to_date' AND d.etatcmd='$typefactures'";
        ...
        $RSql = "SELECT  c.designations as client,f.id, f.dates, f.types, f.id_client, f.observation, f.numeros, f.etatcmd, f.totalht, f.tfrais, f.ttransport, f.ncommande, f.datecmd FROM `tdocuments` f INNER JOIN `tclients` c ON f.id_client = c.id  WHERE dates BETWEEN '$from_date' AND '$to_date' AND etatcmd='$typefactures' GROUP BY  c.designations, c.designations, f.id, f.dates, f.types, f.id_client, f.observation, f.numeros, f.typesfacture, f.totalht, f.tfrais, f.ttransport ORDER BY datecmd  LIMIT :premier, :parpage ";
        $stmt = $conn->prepare($RSql);
        $stmt->bindValue(':premier', $premier, PDO::PARAM_INT);
        $stmt->bindValue(':parpage', $parPage, PDO::PARAM_INT);
        $stmt->execute();
        ...
        <div class="well">
            <center>
                <div style="border: 1px solid black; margin: 5px;">
                    <h1 style="text-shadow: -2px -2px white,2px 2px #0F0604; color:#504846;">Liste des Commandes en attentes</h1>
                </div>
                        ...
                        <tr>
                            ...
                            <td style=" font-family: sans-serif ; text-align: center; padding: 3px; ">
                                <?php
                                    ...
                                    echo '<a type="button" onclick="test(' . $serverId . ', \'' . $serverEtatcmd . '\')"  data-toggle="tooltip" data-placement="top" title="' . htmlspecialchars($tooltipText1, ENT_QUOTES, 'UTF-8') . '"><i class="fa fa-pencil fa-2x" style="font-size:20px;"></i></a>';
