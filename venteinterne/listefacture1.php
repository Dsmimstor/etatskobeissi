...
if (!isset($_SESSION["users"])) {
    ...
    $typefactures = 'PR'; //$_GET["typefactures"];
    //}
    if ($typefactures == "T") {
        ...
        $RSql = "SELECT  c.designations as client,f.id, f.dates, f.types, f.id_client, f.observation, f.numeros, f.typesfacture, f.totalht, f.tfrais, f.ttransport, f.etatcmd FROM `tdocuments` f INNER JOIN `tclients` c ON f.id_client = c.id  WHERE dates BETWEEN '$from_date' AND '$to_date' GROUP BY  c.designations, c.designations, f.id, f.dates, f.types, f.id_client, f.observation, f.numeros, f.etat, f.totalht, f.tfrais, f.ttransport, f.etatcmd ORDER BY dates  LIMIT :premier, :parpage ";
        ...
        $typefactures = "FA";
        $TSql = "SELECT COUNT(*) AS nb_enregistrement FROM `tdocuments` d INNER JOIN `tclients` c ON d.id_client = c.id WHERE d.dates BETWEEN '$from_date' AND '$to_date' AND d.typesfacture='$typefactures'";
        ...
        $RSql = "SELECT  c.designations as client,f.id, f.dates, f.typesfacture, f.id_client, f.observation, f.numeros, f.etatcmd, f.totalht, f.tfrais, f.ttransport FROM `tdocuments` f INNER JOIN `tclients` c ON f.id_client = c.id  WHERE dates BETWEEN '$from_date' AND '$to_date' AND typesfacture='$typefactures' GROUP BY  c.designations, c.designations, f.id, f.dates, f.types, f.id_client, f.observation, f.numeros, f.typesfacture, f.totalht, f.tfrais, f.ttransport ORDER BY dates  LIMIT :premier, :parpage ";
        $stmt = $conn->prepare($RSql);
        $stmt->bindValue(':premier', $premier, PDO::PARAM_INT);
        $stmt->bindValue(':parpage', $parPage, PDO::PARAM_INT);
        $stmt->execute();
                ...
                <tbody>
                    ...
                    ?>
                        <tr>
                            ...
                            <td style=" font-family: sans-serif ; text-align: center; padding: 3px; ">
                                <a type="button" onclick="test(<?php echo $server['id']; ?>, '<?php echo $server['etatcmd']; ?>')" class="edit"><i class="fa fa-pencil fa-2x" style="font-size:20px;"></i></a>
                                <a type="button" href="commandesup.php?id=<?php echo $server["id"]; ?>" class="edit"><i class="fa fa-edit" style="font-size:20px; color:green;"></i></a>
                                <a type="button" onclick="confirmer(<?php echo $server['id']; ?>)" class="supp"><i class="fa fa-trash fa-2x" style="font-size:20px; color:red; "></i></a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
