...
if (!isset($_SESSION["users"])) {
    ...
    if ($typefactures == "T") {
        ...
        $RSql = "SELECT coalesce(SUM(dd.quantite), 0) HT, coalesce(SUM(dd.trans), 0) trans, coalesce(SUM(dd.frais), 0) frais,  c.designations as client,f.id, f.dates, f.type, f.id_client, f.observation, f.numero, f.etat FROM `tdocuments` f INNER JOIN `tclients` c ON f.id_client = c.id LEFT JOIN tdetail_document dd ON dd.id_document=f.id WHERE dates BETWEEN '$from_date' AND '$to_date' GROUP BY  c.designations, c.designations, f.id, f.dates, f.type, f.id_client, f.observation, f.numero, f.etat ORDER BY dates  LIMIT :premier, :parpage ";
        ...
        $TSql = "SELECT COUNT(*) AS nb_enregistrement FROM `tdocuments` d INNER JOIN `tclients` c ON d.id_client = c.id WHERE f.dates BETWEEN '$from_date' AND '$to_date' AND f.type='$typefactures'";
        $stmt = $conn->prepare($TSql);
        $stmt->execute();
        //on recupere le nombre d'element
        $result = $stmt->fetch();
        ...
        //calcul du premier element de la page
        $premier = ($currentPage * $parPage) - $parPage;
        $RSql = "SELECT SELECT coalesce(SUM(dd.prix*dd.quantite), 0) HT, coalesce(SUM(dd.frais), 0) frais, coalesce(SUM(dd.trans), 0) trans,  c.designations as client,f.id, f.dates, f.type, f.id_client, f.observation, f.numero, f.etat FROM `tdocuments` f INNER JOIN `tclients` c ON f.id_client = c.id LEFT JOIN tdetail_document dd ON dd.id_document=f.id WHERE f.dates BETWEEN '$from_date' AND '$to_date'  AND f.type='$typefactures' GROUP BY  c.designations, c.designations, f.id, f.dates, f.type, f.id_client, f.observation, f.numero, f.etat ORDER BY dates  LIMIT :premier, :parpage ";
        $stmt = $conn->prepare($RSql);
        $stmt->bindValue(':premier', $premier, PDO::PARAM_INT);
        $stmt->bindValue(':parpage', $parPage, PDO::PARAM_INT);
    ...
    <div class="container">
        ...
        <div class="well">
                        ...
                        <div class="row input-group ">
                            <div class="col-md-4">
                                <div class="form-group input-group" style="margin: 5px;">
                                    ...
                                    <select id="typefacture" name="typefactures" onchange="typesd()" size="1" style="width:180px;" class="form-control">
                        ...
                        <tr>
                            ...
                            <td style="font-family: sans-serif ; text-align: center; padding: 3px; ">
                                <span class="border border-primary" id="ttc">
                                    <?= $server["totalht"] + $server["trans"] + $server["frais"] ?>
                                </span>
                            </td>

                            <td style=" font-family: sans-serif ; text-align: center; padding: 3px; ">
                                <a type="button" href="update.php?id=<?php echo $server["id"]; ?>" class="edit"><i class="fa fa-edit" style="font-size:20px"></i></a>
            ...
            <div class="modal-footer">
                <a href="/venteinterne/commandes.php" class="btn btn-primary"><span class="glyphicon glyphicon-plus" style="color:yellow"></span> Commande</a>
                <a href="/venteinterne/detailventebl.php" class="btn btn-primary"><span class="glyphicon glyphicon-plus" style="color:blue"></span> BL</a>
                <a href="/venteinterne/detailventefacture.php" class="btn btn-primary"><span class="glyphicon glyphicon-plus" style="color:white"></span> FACTURE</a>
                <a href="/venteinterne/detailventeavoir.php" class="btn btn-primary"><span class="glyphicon glyphicon-plus" style="color:red"></span> AVOIR</a>
