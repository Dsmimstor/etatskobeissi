...
if (!isset($_SESSION["users"])) {
    ...
    if ($typefactures == "T") {
        ...
        $RSql = "SELECT  c.designations as client,f.id, f.dates, f.typesavoir, f.id_client, f.observation, f.numeros, f.etatcmd, f.totalht, f.tfrais, f.ttransport FROM `tdocuments` f INNER JOIN `tclients` c ON f.id_client = c.id  WHERE dates BETWEEN '$from_date' AND '$to_date' AND typesavoir='$typefactures' GROUP BY  c.designations, c.designations, f.id, f.dates, f.typesbl, f.id_client, f.observation, f.numeros, f.typesfacture, f.totalht, f.tfrais, f.ttransport ORDER BY dates  LIMIT :premier, :parpage ";
        $stmt = $conn->prepare($RSql);
        $stmt->bindValue(':premier', $premier, PDO::PARAM_INT);
        $stmt->bindValue(':parpage', $parPage, PDO::PARAM_INT);
        $stmt->execute();
        ...
        <div class="well">
            <center>
                <div style="border: 1px solid black; margin: 5px;">
                    <h1 style="text-shadow: -2px -2px white,2px 2px #0F0604; color:#504846;">Liste des Factures Avoir</h1>
                </div>
