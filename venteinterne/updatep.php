...
if (!isset($_SESSION["users"])) {
    ...
    $id = $_GET["id"];
    $RSqlncmdup = "SELECT * FROM `tdetail_document` WHERE `id` = '$id' ";
    $queryncmdup = $conn->prepare($RSqlncmdup);
    $queryncmdup->execute();
    $resultncmdup = $queryncmdup->fetch(PDO::FETCH_ASSOC);

    $iddocument = $resultncmdup["id_document"];
    $RSqlncmd = "SELECT * FROM `tdocuments` WHERE `id` = '$iddocument' ";
    ...
    if (isset($_POST["btn_update"])) {
        ...
        $RSqls  = "UPDATE `tdetail_document` SET
                            `prix`=  :prix,
                            `quantite`= :quantite,
        ...
        $query->bindValue(":id_document", $iddocument, PDO::PARAM_STR);
...
<head>
    ...
    if ($excecuteIsOk == "succes") {
        echo "<script> alert('La commande a été Modifiée');window.location.href='detailventeproforma.php?id=" . $iddocument . "';</script>";
    }
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-body">
                            <div class="container-fluid">
                                ...
                                <div class="form-group input-group">
                                    ...
                                    <input type="text" onkeydown="upperCaseFun(this)" style="width:350px;" class="form-control" id="articles" name="articles" value="<?php echo $resultncmdup["articles"]; ?>">
                                </div>
                                <div class="form-group input-group">
                                    <span class="input-group-addon" style="width:150px;">Site :</span>
                                    <input type="text" style="width:350px;" class="form-control" id="sites" name="sites" oninput="myFunction();" value="<?php echo $resultncmdup["sites"]; ?>">
                                </div>
                                <div class="form-group input-group">
                                    <span class="input-group-addon" style="width:150px;">Prix :</span>
                                    <input type="number" style="width:350px;" class="form-control" id="prix" name="prix" oninput="calculate()" value="<?php echo $resultncmdup["prix"]; ?>">
                                </div>
                                <div class="form-group input-group">
                                    <span class="input-group-addon" style="width:150px;">Quantité :</span>
                                    <input type="number" style="width:350px;" class="form-control" id="quantite" name="quantite" oninput="calculate()" value="<?php echo $resultncmdup["quantite"]; ?>">
                                </div>
                                <div class="form-group input-group">
                        ...
                        <div class="modal-footer">
                            ...
                            <a type="button" href="/venteinterne/detailventeproforma.php?id=<?php echo $iddocument; ?>" class="btn btn-default"><span class="glyphicon glyphicon-remove"></span> Annuler</a>
                        </div>
                    </div>
                </div>
            </form>
