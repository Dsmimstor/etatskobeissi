...
if (!isset($_SESSION["users"])) {
    ...
    $typefactures = 'Emis'; //$_GET["typefactures"];
    //}
    if ($typefactures == "T") {
        ...
        $RSql = "SELECT  c.designations as client,f.id, f.dates, f.types, f.id_client, f.observation, f.numeros, f.etatcmd, f.totalht, f.tfrais, f.ttransport, f.etatcmd FROM `tdocuments` f INNER JOIN `tclients` c ON f.id_client = c.id  WHERE dates BETWEEN '$from_date' AND '$to_date' GROUP BY  c.designations, c.designations, f.id, f.dates, f.types, f.id_client, f.observation, f.numeros, f.totalht, f.tfrais, f.ttransport, f.etatcmd ORDER BY dates  LIMIT :premier, :parpage ";
        ...
        $TSql = "SELECT COUNT(*) AS nb_enregistrement FROM `tdocuments` d INNER JOIN `tclients` c ON d.id_client = c.id WHERE d.dates BETWEEN '$from_date' AND '$to_date' AND d.etatcmd='$typefactures'";
        //  $RSql = "SELECT  c.designations as client,f.id, f.dates, f.types, f.id_client, f.observation, f.numeros, f.etat, f.totalht, f.tfrais, f.ttransport, f.etatcmd FROM `tdocuments` f INNER JOIN `tclients` c ON f.id_client = c.id  WHERE dates BETWEEN '$from_date' AND '$to_date' AND f.types='$typefactures GROUP BY  c.designations, c.designations, f.id, f.dates, f.types, f.id_client, f.observation, f.numeros, f.etat, f.totalht, f.tfrais, f.ttransport, f.etatcmd ORDER BY dates  LIMIT :premier, :parpage ";
        ...
        $RSql = "SELECT  c.designations as client,f.id, f.dates, f.types, f.id_client, f.observation, f.numeros, f.etatcmd, f.totalht, f.tfrais, f.ttransport FROM `tdocuments` f INNER JOIN `tclients` c ON f.id_client = c.id  WHERE dates BETWEEN '$from_date' AND '$to_date' AND etatcmd='$typefactures' GROUP BY  c.designations, c.designations, f.id, f.dates, f.types, f.id_client, f.observation, f.numeros, f.typesfacture, f.totalht, f.tfrais, f.ttransport ORDER BY dates  LIMIT :premier, :parpage ";
        $stmt = $conn->prepare($RSql);
        $stmt->bindValue(':premier', $premier, PDO::PARAM_INT);
        $stmt->bindValue(':parpage', $parPage, PDO::PARAM_INT);
        $stmt->execute();
        ...
        <div class="well">
            <center>
                <div style="border: 1px solid black; margin: 5px;">
                    <h1 style="text-shadow: -2px -2px white,2px 2px #0F0604; color:#504846;">Liste des Proforma</h1>
                </div>
                        ...
                        <div class="row input-group ">
                            <div class="col-md-4">
                                <div class="form-group input-group" style="margin: 5px;">
                                    <span class="input-group-addon" style="width:240px;"> A la Date :</span name="etat">
                    ...
                    ?>
                        <tr>
                            ...
                            <td style=" font-family: sans-serif ; text-align: center; padding: 3px; ">
                                ...
                                <?php
                                $tooltipText = "Modifier la Proforma";
                                $serverId = htmlspecialchars($server['id'], ENT_QUOTES, 'UTF-8');
                                $serverEtatcmd = htmlspecialchars($server['etatcmd'], ENT_QUOTES, 'UTF-8');

                                echo '<a type="button" onclick="test(' . $serverId . ', \'' . $serverEtatcmd . '\')" class="fa fa-pencil fa-2x" style="font-size:20px;" data-toggle="tooltip" data-placement="top" title="' . htmlspecialchars($tooltipText, ENT_QUOTES, 'UTF-8') . '"></a>';
                                ?>

                                <?php
                                $tooltipText = "Modifier le Client";
                                echo  '<a type="button" href="commandesup.php?id=<?php echo $server["id"]; ?> <i class="fa fa-edit" style="font-size:20px; color:green;"  data-toggle="tooltip" data-placement="top" title="' . htmlspecialchars($tooltipText, ENT_QUOTES, 'UTF-8') . '"></i></a>';
                                ?>
                                <?php
                                $tooltipText = "Voir la Proforma";
                                $serverId = htmlspecialchars($server['id'], ENT_QUOTES, 'UTF-8');
                                $serverEtatcmd = htmlspecialchars($server['etatcmd'], ENT_QUOTES, 'UTF-8');

                                echo '<a type="button" onclick="test(' . $serverId . ')" class="supp" data-toggle="tooltip" data-placement="top" title="' . htmlspecialchars($tooltipText, ENT_QUOTES, 'UTF-8') . '"><i class="fa fa-search fa-2x" style="font-size:20px;"></i></a>';
                                ?>
                                <?php
                                $tooltipText = "Supprimer la Proforma";
                                $serverId = htmlspecialchars($server['id'], ENT_QUOTES, 'UTF-8');
                                $serverEtatcmd = htmlspecialchars($server['etatcmd'], ENT_QUOTES, 'UTF-8');
            ...
            <div class="modal-footer">
                ...
                ?>
                <?php
                $tooltipText = "Fiste des Factures";
                ...
                ?>
                <a href="/venteinterne/listevente.php" class="btn btn-default"><span class="glyphicon glyphicon-remove"></span> Accueil</a>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function test(idt, etatcmd) {
            if (etatcmd === "Emis") {
                ...
                location.href = "detailproforma.php?id=" + idt;
            }
        }
    </script>
</body>
