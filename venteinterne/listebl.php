...
if (!isset($_SESSION["users"])) {
    ...
    if ($typefactures == "T") {
        ...
        $TSql = "SELECT COUNT(*) AS nb_enregistrement FROM `tdocuments` d INNER JOIN `tclients` c ON d.id_client = c.id WHERE d.datebl BETWEEN '$from_date' AND '$to_date' AND d.typesbl='$typefactures'";
        ...
        $RSql = "SELECT  c.designations as client,f.id, f.datebl, f.nbonliv, f.etatcmd, f.id_client, f.observation, f.totalht, f.tfrais, f.ttransport FROM `tdocuments` f INNER JOIN `tclients` c ON f.id_client = c.id  WHERE datebl BETWEEN '$from_date' AND '$to_date' AND typesbl='$typefactures' GROUP BY  c.designations, f.id, f.datebl, f.typesbl, f.id_client, f.observation, f.nbonliv, f.typesfacture, f.totalht, f.tfrais, f.ttransport ORDER BY datebl  LIMIT :premier, :parpage ";
        $stmt = $conn->prepare($RSql);
        $stmt->bindValue(':premier', $premier, PDO::PARAM_INT);
        $stmt->bindValue(':parpage', $parPage, PDO::PARAM_INT);
        $stmt->execute();
        ...
        <div class="well">
            <center>
                <div style="border: 1px solid black; margin: 5px;">
                    <h1 style="text-shadow: -2px -2px white,2px 2px #0F0604; color:#504846;">Liste des Bons de Livraisons</h1>
                </div>
                        ...
                        <tr>
                            ...
                            <td style="text-align: center; font-family: sans-serif; padding: 3px; ">
                                <span class="border border-primary" id="dates">
                                    <?= date('d-m-Y', strtotime($server["datebl"])); ?>
                                </span>
                            </td>
                            ...
                            <td style=" font-family: sans-serif ; text-align: center; padding: 3px; ">
                                ...
                                <?php
                                $tooltipText = "Basculer le BL en Facture";
                                ...
                                ?>
                                <?php
                                $tooltipText = "Voir le Bon de Livraison";
                                $serverId = htmlspecialchars($server['id'], ENT_QUOTES, 'UTF-8');
                                $serverEtatcmd = htmlspecialchars($server['etatcmd'], ENT_QUOTES, 'UTF-8');
                                echo '<a type="button" onclick="test1(' . $serverId . ')" class="supp" data-toggle="tooltip" data-placement="top" title="' . htmlspecialchars($tooltipText, ENT_QUOTES, 'UTF-8') . '"><i class="fa fa-search fa-2x" style="font-size:20px;"></i></a>';
                                ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
    ...
    <script>
        ...
        function test1(idt, etatcmd) {
            if (etatcmd === "Emis") {
                ...
                location.href = "detailblvalider.php?id=" + idt;
            }
        }
    </script>
</body>
